...
<p style="color:#003D79;font-size:16px">**This notebook would introduce the basic syntax and `dplyr` package of R Programming. Before digging into Spatial Analysis, let us be familiar with R. The installation of R and R Studio please see [here]()**</p>
...
Then we can merge these two data by using the country name. Note that the country name in data `world` and `world_coffee` are all `name_long`, and hence, we can simply set `by="`, or even without setting it (the package will automatically find the common name to be the key.)  
